                                        ...
                                        const float eps)
{
    ...
    for(unsigned int i = lid; i < INNER_SIZE; i += LOCAL_SIZE)
    {
        ...
        float pdy_pweight = hip_kernel_provider::cast<float>(dy[idx])
                            * hip_kernel_provider::cast<float>(scale[i]);

        sum_dy_scale += pdy_pweight;
        sum_dy_scale_x += pdy_pweight * hip_kernel_provider::cast<float>(x[idx]);
    }
...
extern "C" __global__ void LayernormBwdWeightBias(const OutputType* __restrict__ dy,
                                                  const InputType* __restrict__ x,
                                                  const MeanInvVarianceType* __restrict__ mean,
                                                  const MeanInvVarianceType* __restrict__ rstd,
...
extern "C" __global__ void
    LayernormBwdWeightBiasParallel(const OutputType* __restrict__ dy,
                                   const InputType* __restrict__ x,
                                   const MeanInvVarianceType* __restrict__ mean,
                                   const MeanInvVarianceType* __restrict__ rstd,
                                   float* __restrict__ workspace)
...
extern "C" __global__ void LayernormBwdWeightBiasReduceSum(const float* __restrict__ workspace,
                                                           ScaleBiasType* __restrict__ dscale,
                                                           ScaleBiasType* __restrict__ dbias)
{
    const unsigned int gid = threadIdx.x + blockIdx.x * LOCAL_SIZE;
