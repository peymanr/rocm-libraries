...
- **Tensor loop policy** — by default, exact `CK_Tensor` queues are frozen after the first solver sweep so tagged tensor tokens do not propagate around loop back-edges; blocks with untagged tensor anchors keep their live tensor queues because those anchors are fences, and a conservative option restores normal tensor fixed-point iteration
